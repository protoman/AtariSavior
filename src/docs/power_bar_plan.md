...
- [ ] **1a.** Replace timer bar section in `src/bank1.asm` (lines 96-148):
      remove player-sprite loop (NUSIZ0/GRP0), keep CTRLPF=$05.
- [ ] **1b.** Set PF0=$70, PF1=$FF, PF2=$FF (bar shape with edge margins).
...
- [ ] **1d.** After bar: clear PF0/PF1/PF2, restore COLUPF for HUD.
- [ ] **1e.** Remove unused `BarPF*Table` lookups (lines 595-647).
- [ ] **1f.** Scanline budget: bar 6→3 lines (saves 3, HUD band ≤48).
- [ ] **1g.** Build (`./build.sh`), user tests in Stella:
      full yellow → drains right→left with red, margins visible.
