...
- **Start screen (bank1):** a self-contained menu that owns its vsync/vblank/kernel/
  overscan and loops on itself until Fire (INPT4 bit7 active low) sets `GameMode=1`
  and `jmp ToGameStub`. Frame = VSYNC 3 + VBLANK 38 + kernel 192 + overscan 29
  (= bank0's 262-line lock, OVSCAN_TIME=33). The screen is a 40x192 asymmetric
  playfield: CTRLPF=0 (no reflection), left-half PF writes during HBLANK then
  mid-line right-half rewrites (PF0R lands ~cyc30, PF1R ~cyc39, PF2R ~cyc50 —
  after the left risers latch, before the right risers start). Bitmaps are
  generated by `tools/start_screen.py` into `generated/start_screen.asm` (six
  192-byte tables, one per register/half, each `.align 256` so indexed loads
  never cross pages). Text is rendered UPPERCASE from an embedded 5x7 font with
  gap-less derived glyphs; hero art is drawn with rectangle primitives.
- **Cross-bank fold pads (F6):** bank0 and bank1 replicate identical bytes at the
  ...
  `$fd00 ToMenuStub = lda #1 / sta $1FF7 / jmp MenuMain` (bank0->menu, taken when
  `GameMode==0` in StartFrame) and `$fe00 ToGameStub = lda #0 / sta $1FF6 / jmp
